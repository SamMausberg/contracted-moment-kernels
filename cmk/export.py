...
def export_rows(envelopes):
    ...
    for j in range(len(envelopes[0].center)):
        ...
        boxes=[[outward(e.mass.lo,False),outward(e.mass.hi,True),
                outward(e.central[j].lo,False),outward(e.central[j].hi,True),
                outward(e.center[j],False),outward(e.center[j],True)]
               for e in envelopes]
