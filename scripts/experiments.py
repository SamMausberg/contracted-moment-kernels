...
def suite():
    ...
    for name, active, discarded, mixed in [
        ...
        for r, diag, sharp in [(d, False, False), (d, True, True), (4, True, True)]:
            ...
            s = summarize(k, v, groups, rank=r, keep_diagonal=diag)
            ...
            row = dict(
                ...
                keep_diagonal=diag,
                sharp_tail=sharp,
                full_gate_passes=passes,
                coordinate_gate_passes=coords,
                queries=Q,
                coordinates=Q * h,
                summary_bytes=s.nbytes,
                kv_bytes=k.nbytes + v.nbytes,
                setup_ms=setup_ms,
                dense_ms_per_query=dense_ms,
                evaluation_ms_per_query=eval_ms,
            ...
            if diag and sharp:
                ...
                for qi, q in enumerate(qset):
                    ...
                    scanned.append(a["scanned_tokens"])
                    accepted += int(a["accepted"].all())
                row.update(
                    adaptive_full_gate_passes=accepted,
    ...
    return dict(
        ...
        dimensions=dict(N=n, d=d, h=h, B=B, Q=Q),
        ablations=results,
        rational_example=counts,
        tail_old_over_new={
            str(x): tail_coefficient(x, False) / tail_coefficient(x, True)
