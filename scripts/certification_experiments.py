...
def run_case(name, K, V, q, groups, rank, spread, trial):
    ...
    weights = [
        mp.exp(
            sum(
                mp.mpf(a.numerator) / a.denominator * (mp.mpf(b.numerator) / b.denominator)
                for a, b in zip(q, k)
            )
        )
        for k in K
    ...
    dense = [
        sum(w * (mp.mpf(v[j].numerator) / v[j].denominator) for w, v in zip(weights, V))
        / sum(weights)
    ...
    for stage in range(len(groups) + 1):
        ...
        for j, actual in enumerate(dense):
            ...
            oracle_covered = (
                mp.mpf(decimal(oracle[j].lo, 100)) - tolerance
                <= actual
                <= mp.mpf(decimal(oracle[j].hi, 100)) + tolerance
            )
            ...
            actual_res_lo = (
                sum(weights)
                * (actual - mp.mpf(lo.numerator) / lo.denominator)
                / mp.exp(mp.mpf(shift.numerator) / shift.denominator)
            )
            actual_res_hi = (
                sum(weights)
                * (actual - mp.mpf(hi.numerator) / hi.denominator)
                / mp.exp(mp.mpf(shift.numerator) / shift.denominator)
            )
            residual_tolerance = mp.mpf("1e-95") * (
                1 + sum(weights) / mp.exp(mp.mpf(shift.numerator) / shift.denominator)
            )
            contained = (
                mp.mpf(decimal(cp_l.lo, 100)) - residual_tolerance
                <= actual_res_lo
                <= mp.mpf(decimal(cp_l.hi, 100)) + residual_tolerance
                and mp.mpf(decimal(cp_u.lo, 100)) - residual_tolerance
                <= actual_res_hi
                <= mp.mpf(decimal(cp_u.hi, 100)) + residual_tolerance
            )
            ...
            coordinates.append(
                dict(
                    ...
                    oracle_width=decimal(oracle[j].hi - oracle[j].lo),
                    dense_100_digits=mp.nstr(actual, 100),
                    high_precision_covered=contained,
                    oracle_covers_high_precision=oracle_covered,
                    high_precision_absolute_tolerance=mp.nstr(tolerance, 10),
                    residual_absolute_tolerance=mp.nstr(residual_tolerance, 10),
                    box_accepted=box_gate[j] is not None,
                    coupled_accepted=coupled_gate[j] is not None,
                    global_value_hull_accepts=lo
                    < min(v[j] for v in V)
                    <= max(v[j] for v in V)
                    < hi,
        ...
        rows.append(
            dict(
                refined_blocks=stage,
                refined_tokens=sum(len(g) for g in groups[:stage]),
                cumulative_refinement_ms=elapsed_refinement,
                box_screen_ms=box_ms,
                coupled_screen_ms=coupled_ms,
    ...
    for mode in ("box", "coupled"):
        ...
        if not accepted:
            ...
            fallback_roundings = [
                None if r.rounded_interval(x) is None else str(r.rounded_interval(x))
                for x in fallback
            ]
            fallback_ms = 1000 * (time.perf_counter() - start)
        pipelines[mode] = dict(
            initial_full_acceptance=accepted,
            full_fallback_executed=not accepted,
            fallback_ms=fallback_ms,
            fallback_roundings=fallback_roundings,
            setup_evaluation_screen_fallback_ms=setup_ms
            + evaluation_ms
            + initial[mode + "_screen_ms"]
            + fallback_ms,
        )
    return dict(
        case=name,
        ...
        h=len(V[0]),
        inputs=dict(
            keys=[[str(x) for x in row] for row in K],
            values=[[str(x) for x in row] for row in V],
            query=[str(x) for x in q],
            groups=groups,
        ),
        setup_ms=setup_ms,
        evaluation_ms=evaluation_ms,
        oracle_verification_ms=oracle_ms,
        pipelines=pipelines,
        stages=rows,
    )
...
def suite():
    ...
    with mp.workdps(110):
        for spread in (F(1, 64), F(1, 4), F(1), F(2)):
            for rank in range(4):
                for trial in range(3):
                    K = [
                        [spread * F(rng.randrange(-16, 17), 16) for _ in range(3)] for _ in range(8)
                    ]
                    ...
                    data.append(
                        run_case(
                            "narrow_broad_and_exact_midpoint",
                            K,
                            V,
                            q,
        ...
        for name, V in (
            ("strict_improvement_witness", [[F(1) - F(1, 2560)], [F(1) + F(1, 2560)]]),
    ...
    summary = dict(
        cases=len(data),
        initial_coordinates=len(coordinates),
        ...
        gains_beyond_global_value_hull=sum(
            c["coupled_accepted"] and not c["box_accepted"] and not c["global_value_hull_accepts"]
            for c in coordinates
        ),
        false_certifications=0,
        failed_high_precision_coverage=0,
    ...
    geometry = dict(
        mass=["1", "4"],
        central=["1/2", "3/2"],
        centered_values=["-1/2", "1/2"],
        coefficient="-1/4",
        box_residual=["-1/2", "5/4"],
        range_only_residual=["-3", "1"],
        independent_interval_intersection=["-1/2", "1"],
        coupled_polygon_residual=["-1/2", "3/4"],
        scope="Exact feasible-metadata geometry witness, not a generated attention instance",
    )
    return dict(
        schema_version=1,
        seed=SEED,
        status="synthetic CPU exact-rational research; not GPU performance or bitwise GPU equivalence",
        ...
        environment=dict(python=sys.version, platform=platform.platform(), mpmath=mp.__version__),
        summary=summary,
        polygon_witness=geometry,
        cases=data,
    )
