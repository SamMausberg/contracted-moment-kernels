...
\begin{equation}
 \begin{gathered}
 y_j=\frac{N_j}{Z},\qquad N_j=\sum_b n_{bj},\\
 Z=\sum_b z_b>0.
 \end{gathered}
...
\begin{align}
 \mathcal L_j(c)&=\sum_b\Bigl[m^-_{bj}\notag\\
 &\qquad+\min\{(\nu_{bj}-c)l_b,(\nu_{bj}-c)u_b\}\Bigr],\label{eq:lower-residual}\\
 \mathcal U_j(c)&=\sum_b\Bigl[m^+_{bj}\notag\\
 &\qquad+\max\{(\nu_{bj}-c)l_b,(\nu_{bj}-c)u_b\}\Bigr].\label{eq:upper-residual}
\end{align}
...
\begin{equation}
 ...
 &l_b\le z\le u_b,\quad m^-_{bj}\le m\le m^+_{bj},\\
...
\begin{gather}
 a_b=q^T\mu_b,\qquad w_b=n_b e^{a_b-s},\notag\\
 \sigma_b^2=u^T\Sigma_bu,\qquad A_b=1+\sigma_b^2/2,\notag\\
 \tau_b=\kappa(\rho_b)\sigma_b^2,
 \label{eq:moment-scalars}\\
 \widetilde l_b=w_b\max\{1,A_b-\tau_b\},\notag\\
 \widetilde u_b=w_b(A_b+\tau_b),\label{eq:projected-mass}\\
...
\begin{gather}
 ...
 \begin{aligned}
 m^\pm_{bj}&=\widehat m_{bj}\\
 &\quad\pm\left[\beta_{bj}+(e^{\epsilon_b}-1)\widetilde u_bR_{bj}\right].
 \end{aligned}
