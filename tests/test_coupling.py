...
def polygon_oracle(z, c, lower, upper, coefficient):
    ...
    constraints = [(-1,0,-z.lo),(1,0,z.hi),(0,-1,-c.lo),(0,1,c.hi),
                   (lower,-1,0),(-upper,1,0)]
    objectives = []
...
def test_coupled_support_matches_independent_halfplane_oracle():
    ...
    for _ in range(500):
        ...
        checked += 1
    assert checked == 500
    assert r.coupled_block_residual(r.Interval(1,4),r.Interval(-2,2),F(-1,2),F(1,2),1) == r.Interval(F(1,2),6)
    ...
    assert r.coupled_block_residual(r.Interval(1,4),r.Interval(F(1,2),F(3,2)),
                                    F(-1,2),F(1,2),F(-1,4)) == r.Interval(F(-1,2),F(3,4))
...
def test_coupled_residual_dense_high_precision_and_refinement():
    ...
    with mp.workdps(100):
        for trial in range(40):
            ...
            s = r.summarize(K,V,[range(4),range(4,8)],rank=trial%4,keep_diagonal=trial%2==0)
            e, shift = r.evaluate(q,s,bits=72)
            weights = [mp.exp(sum(high(x)*high(y) for x,y in zip(q,k))-high(shift)) for k in K]
            for j in range(2):
...
def test_numerical_coupling_matches_rational_geometry():
    ...
    for _ in range(40):
        ...
        for j in range(2):
            ...
            for b in range(2):
                sums += polygon_oracle(r.Interval(e.zlo[b],e.zhi[b]),r.Interval(e.mlo[b,j],e.mhi[b,j]),
                                       F(e.value_lower[b,j]),F(e.value_upper[b,j]),F(e.nu[b,j]-boundary[j]))
            exact_low.append(float(sums.lo));exact_high.append(float(sums.hi))
