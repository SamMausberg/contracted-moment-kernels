...
struct BlockEnvelope { double zlo, zhi, mlo, mhi, zhat, mhat; };
struct ScreenResult {
  double candidate, rounded, lower, upper, lower_residual, upper_residual;
  int numerical_accept;
};
CMK_HD inline bool finite(double x) { return x == x && fabs(x) <= DBL_MAX; }
CMK_HD inline double sharp_tail(double x) {
  if (!finite(x) || x < 0 || x > 650) return HUGE_VAL;
  if (x == 0) return 0;
  if (x < .5) {
    double term = x/6, total = term;
    for (int k=4; k<36; ++k) { term *= x/k; total += term; }
    return total;
  }
  return (expm1(x)-x-x*x/2)/(x*x);
}
CMK_HD inline BlockEnvelope evaluate_block(SummaryView s, const double* q,
                                         int b, int j, double shift) {
  double score=0, rho=0, eps=0, variance=0, q2=0, c=0;
  for (int a=0; a<s.d; ++a) {
    score += q[a]*s.mu[b*s.d+a];
    if (a<s.r) rho += fabs(q[a])*s.kr[b*s.d+a];
    else eps += fabs(q[a])*s.kr[b*s.d+a];
  }
  for (int a=0; a<s.r; ++a) {
    q2 += q[a]*q[a];
    ...
    for (int k=0; k<s.r; ++k)
      variance += q[a]*q[k]*s.cov[(b*s.r+a)*s.r+k];
  }
...
CMK_HD inline double bf16_nearest(double x) {
  ...
  if (x==0) return 0;
  double a=fabs(x); int exponent; frexp(a,&exponent); --exponent;
  int p=exponent-7; if (p < -133) p=-133;
...
CMK_HD inline void bf16_cell(double b,double& lo,double& hi) {
  if (b==0) { lo=-ldexp(1.,-134);hi=ldexp(1.,-134);return; }
  double a=fabs(b);int e;frexp(a,&e);--e;
  int p=e-7;if(p < -133)p=-133;
  double step=ldexp(1.,p);
  double prev=(a==ldexp(1.,e) && e > -126)? step/2:step;
  double l=a-prev/2,u=a+step/2;
  if(b<0){lo=-u;hi=-l;}else{lo=l;hi=u;}
}
CMK_HD inline ScreenResult screen_row(SummaryView s,const BlockEnvelope* boxes,int j) {
  double den=0,num=0,masslo=0; bool ok=true;
  for(int b=0;b<s.B;++b){
    BlockEnvelope e=boxes[b*s.h+j];double nu=s.nu[b*s.h+j];
    den+=e.zhat;num+=nu*e.zhat+e.mhat;masslo+=e.zlo;
    ok=ok && finite(e.zlo)&&finite(e.zhi)&&finite(e.mlo)&&finite(e.mhi);
  }
  ...
  return {candidate,rounded,lo,hi,rl,ru,
