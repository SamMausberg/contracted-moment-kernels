...
int main(int argc, char** argv) {
  try {
    ...
    for (int q = 0; q < f.Q; ++q)
      for (int b = 0; b < f.B; ++b)
        for (int j = 0; j < f.h; ++j) {
          size_t p = (q * f.B + b) * f.h + j;
          auto e = cmk::evaluate_block(s, f.queries.data() + q * f.d, b, j, f.shifts[q]);
          boxes[p] = e;
          double xs[] = {e.zlo, e.zhi, e.mlo, e.mhi, e.zhat, e.mhat};
          for (int k = 0; k < 6; ++k) {
            double want = f.expected_boxes[p * 6 + k];
            if (!cmk::finite(xs[k]) || !cmk::finite(want))
    ...
    for (int q = 0; q < f.Q; ++q)
      for (int j = 0; j < f.h; ++j) {
        ...
        if (x.numerical_accept != f.expected_gate[q * f.h + j])
          throw std::runtime_error("screen mismatch");
        double y = f.expected_y[q * f.h + j];
        if (x.numerical_accept && !(x.lower < y && y < x.upper))
          throw std::runtime_error("false numerical screen");
