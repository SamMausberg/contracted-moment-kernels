...
#include <limits>
namespace cmk {
struct ImportedBox {double zlo,zhi,mlo,mhi,centerlo,centerhi;};
#ifndef __CUDACC__
...
inline bool check_imported(const ImportedBox* boxes,int B,double lower,double upper){
 ...
 for(int b=0;b<B;++b){const auto x=boxes[b];
  valid=valid&&std::isfinite(x.zlo)&&std::isfinite(x.zhi)&&std::isfinite(x.mlo)&&std::isfinite(x.mhi)
    &&std::isfinite(x.centerlo)&&std::isfinite(x.centerhi)&&x.zlo>=0&&x.zlo<=x.zhi
  ...
  valid=valid&&std::isfinite(al)&&std::isfinite(au)&&std::isfinite(bl)&&std::isfinite(bu);
