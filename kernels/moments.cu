...
// Research CUDA kernels. See docs/GH200.md for recorded validation and limits.
#include <cuda_runtime.h>

#include "directed.cuh"
#include "moments.cuh"
namespace cmk {
__global__ void evaluate_blocks_shared(SummaryView s, const double* q, const double* shift, int Q,
                                       BlockEnvelope* out) {
  int b = blockIdx.x % s.B, qi = blockIdx.x / s.B;
  if (qi >= Q) return;
  __shared__ BlockScalar scalar;
  if (threadIdx.x == 0) scalar = evaluate_scalar(s, q + qi * s.d, b, shift[qi]);
  __syncthreads();
  for (int j = threadIdx.x; j < s.h; j += blockDim.x)
    out[(size_t(qi) * s.B + b) * s.h + j] = evaluate_channel(s, q + qi * s.d, b, j, scalar);
}
__global__ void evaluate_blocks(SummaryView s, const double* q, const double* shift, int Q,
                                BlockEnvelope* out) {
...
__global__ void reduce_and_screen(SummaryView s, const BlockEnvelope* in, int Q,
                                  ScreenResult* out) {
...
__global__ void scan_selected_blocks(SummaryView s, const double* q, const double* shift,
                                     const double* K, const double* V, const int* offsets,
                                     const int* selected, int Q, BlockEnvelope* out) {
  size_t p = size_t(blockIdx.x) * blockDim.x + threadIdx.x;
  if (p >= size_t(Q) * s.B * s.h) return;
  int j = p % s.h, b = (p / s.h) % s.B, qi = p / (s.B * s.h);
  if (!selected[qi * s.B + b]) return;
  double z = 0, m = 0;
  for (int i = offsets[b]; i < offsets[b + 1]; ++i) {
    double score = 0;
    for (int a = 0; a < s.d; ++a) score += q[qi * s.d + a] * K[i * s.d + a];
    double w = exp(score - shift[qi]);
    z += w;
    m += w * (V[i * s.h + j] - s.nu[b * s.h + j]);
  }
  out[p] = {z, z, m, m, z, m};
}
...
__global__ void screen_imported_boxes(const ImportedBox* boxes, const double* lower,
  ...
  bool valid = finite(lower[row]) && finite(upper[row]) && lower[row] < upper[row];
  for (int b = 0; b < B; ++b) {
