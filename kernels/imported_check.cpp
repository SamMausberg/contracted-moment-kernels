...
int main(int argc, char** argv) {
  try {
    ...
    int rows = 0, B = 0;
    f >> rows >> B;
    if (!f || rows < 1 || rows > 100000 || B < 1 || B > 100000)
    ...
    for (int row = 0; row < rows; ++row) {
      ...
      bool pass = cmk::check_imported(boxes.data(), B, l, u);
      accepted += pass;
      exact_accepted += expected;
      if (pass && !expected)
        throw std::runtime_error("False certificate vs rational residual oracle");
    }
    std::cout << "{\"imported_checker_pass\":true,\"rows\":" << rows
              << ",\"host_certified_rows\":" << accepted
              << ",\"rational_certified_rows\":" << exact_accepted << "}\n";
    return 0;
